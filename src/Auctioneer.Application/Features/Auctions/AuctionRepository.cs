...
public class AuctionRepository : IRepository<Auction>
{
    ...
    public AuctionRepository(IOptions<AuctioneerDatabaseSettings> dbSettings)
    {
        ...
        _auctionCollection = mongoDatabase.GetCollection<Auction>(dbSettings.Value.MemberCollectionName);
    }
    ...
    public async Task<Auction?> GetAsync(Guid id) =>
        await _auctionCollection.Find(x => x.MemberId == id).FirstOrDefaultAsync();
    ...
    public async Task UpdateAsync(Guid id, Auction updatedEntity) =>
        await _auctionCollection.ReplaceOneAsync(x => x.MemberId == id, updatedEntity);

    public async Task DeleteAsync(Guid id) =>
        await _auctionCollection.DeleteOneAsync(x => x.MemberId == id);
}