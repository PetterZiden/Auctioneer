...
public class DeleteMemberTests
{
    ...
    {
        _memberRepository.DeleteAsync(Arg.Any<Guid>()).Returns(Task.CompletedTask);
    ...
    {
        _memberRepository.DeleteAsync(Arg.Any<Guid>()).Returns(x => throw new Exception("MemberRepository failed"));
        _eventRepository.CreateAsync(Arg.Any<DomainEvent>()).Returns(Task.CompletedTask);
        _unitOfWork.SaveAsync().Returns(Task.CompletedTask);
