...
public class CreateMemberTests
{
    ...
    {
        _memberRepository.CreateAsync(Arg.Any<Member>()).Returns(Task.CompletedTask);
    ...
    {
        _memberRepository.CreateAsync(Arg.Any<Member>()).Returns(x => throw new Exception("MemberRepository failed"));
        _eventRepository.CreateAsync(Arg.Any<DomainEvent>()).Returns(Task.CompletedTask);
        _unitOfWork.SaveAsync().Returns(Task.CompletedTask);
